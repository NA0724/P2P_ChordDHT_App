...
class CreateP2PNode:
    def __init__(self, ip, port):
        ...
        self.id = getHashKey(ip + ":" + str(port))
    ...
    def listenThread(self):
        ...
        while True:
            try:
                ...
                threading.Thread(target=self.peerConnectionThread, args=(connection, address)).start()
            except socket.error:
    ...
    # type 2: ping, 
    # type 3: lookupID, 
    # type 4: update Successor/Predecessor
    def peerConnectionThread(self, connection, address):
        rDataList = pickle.loads(connection.recv(buffer))
        connectionType = rDataList[0]
        if connectionType == 0:
            print("Connection with:", address[0], ":", address[1])
            print("Request to Join network, recevied")
            self.joinNetworkByOtherNode(connection, address, rDataList)
            self.printMenu()
        elif connectionType == 1:
            print("Connection with:", address[0], ":", address[1])
            print("Request to Upload/Download File, recevied")
            choice = rDataList[1]
            filename = rDataList[2]
            fileID = getHashKey(filename)
            # IF client wants to download file
            if choice == 0:
                print("Download request for file:", filename)
                try:
                    # First it searches its own directory. If not found, send does not exist
            ...
            print("Request to Update Predecessor/Successor, recevied")
            if rDataList[1] == 1:
                newSucc = rDataList[2]
                self.succ = newSucc
                self.succID = getHashKey(newSucc[0] + ":" + str(newSucc[1]))
                print("Updated successor to", self.succID)
            else:
                newPred = rDataList[2]
                self.pred = newPred
                self.predID = getHashKey(newPred[0] + ":" + str(newPred[1]))
                print("Updated predecessor to", self.predID)
    ...
    def joinNetworkByOtherNode(self, connection, address, rDataList):
        if rDataList:
            peerIPport = rDataList[1]
            peerID = getHashKey(peerIPport[0] + ":" + str(peerIPport[1]))
            oldPred = self.pred
            # Updating pred
    ...
    def start(self):
        ...
        threading.Thread(target=self.ping_success, args=()).start()
        ...
        while True:
            ...
            if userChoice == "1":
                ...
                self.updateOtherFingerTables()   # Telling others to update their f tables
        ...
        while True:
            ...
            try:
                ...
                if newSuccessorFound:
                    ...
                    self.succID = getHashKey(self.succ[0] + ":" + str(self.succ[1]))
    ...
    def sendJoinRequest(self, ip, port):
        try:
            ...
            self.predID = getHashKey(self.pred[0] + ":" + str(self.pred[1]))
            self.succ = recvIPPort
            self.succID = getHashKey(recvIPPort[0] + ":" + str(recvIPPort[1]))
    ...
    def updateFingerTable(self):
        for i in range(MAX_BITS):
            ...
            recvId = getHashKey(recvIPPort[0] + ":" + str(recvIPPort[1]))
    ...
    def printFingerTable(self):
        ...
        for key, value in self.fingerTable.items(): 
            print("KeyID:", key, "Peers/Nodes(IP,Port):", value)
